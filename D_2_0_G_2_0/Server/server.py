...
class GeneratorSplitStepOne(Module):
    """Generator: comparing two original Generator, strip the last linear layer, it will move to client side
    """
...
class MDGANServer():
    ...
    def __init__(self, client_rrefs, epochs = 300, use_cuda = False, batch_size = 500, n_critic = 5, **kwargs):
        ...
        self.d_output_dim_list = self.d_input_dim_list
        # UPDATE: we don't fix the output dim of generator, it is not the 
        # same dim as the encoded dim for each client
        self.data_dim = int(np.sum(self.d_output_dim_list))
        ...
        self.discriminator = Discriminator(
            self.data_dim+self.cond_vec_length,
...
class MDGANClient():
    ...
    def init_local(self, dim):
        self.local_g = Linear(dim, self.data_dim)
...
def run(rank, world_size, ip, port, dataset, epochs, use_cuda, batch_size, n_critic):
    ...
    if rank == 0:  # this is run only on the server side
        ...
        for worker in range(world_size-1):
            ...
            print("finish initialization on client: ", worker+1)
        # time.sleep(2) 
        print("start to initialize Server!")
