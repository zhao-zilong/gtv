...
def apply_activate(data, output_info):
    '''
    used to activate generator's output.
    '''
    data_t = []
    st = 0
...
class MDGANClient():
    ...
    def __init__(self, dataset, epochs, use_cuda, batch_size, **kwargs):
        ...
        self.raw_df = pd.read_csv("Loan.csv").iloc[:,0:self.split_threshold]
    ...
    #     print("shuffle order: ", order[0:10])
    #     print("train data[0]", self.train_data[0])
    ...
    def init_local(self, dim, division):
        print("dim and self.data_dim, division", dim, self.data_dim, division)
        self.local_g = LocalGenerator(input_dim = dim, local_generator_dim = [division, division], data_dim = self.data_dim)
